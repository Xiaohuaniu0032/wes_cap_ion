...
my ($ppv_vcf,$tsvc_bcfnorm_vcf,$ppv_detail_outfile) = @ARGV;
...
# read ppv file and output each InDel HS's TVC detail info

open O, ">$ppv_detail_outfile" or die;
print O "\#TP_or_FP\tHS_Var\tHS_GT\tTVC_QUAL\tTVC_MLLD\tTVC_STB\tTVC_RBI\tTVC_RESULT\tTVC_FR\tTVC_AO\tTVC_FAO\tTVC_DP\tTVC_FDP\tTVC_AF\tTVC_GT\n";

open IN, "$ppv_vcf" or die;
while (<IN>){
	chomp;
	my @arr = split /\t/;
